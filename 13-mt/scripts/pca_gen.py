...
def pca(bed_prefix, pop_file, output_prefix):
    ...
    r_command += 'legend("topright", legend=c("' + '","'.join(labels) + '"), col=c("' + '","'.join([color_dict[label] for label in labels]) + '"), pch=20, lty="blank");'
    r_command += 'dev.off();'
...
def main():
    ...
    pca(merged_bed_prefix, pop_file, 'pca/images/ToMMo_3.5kjpn_with_EAS')
