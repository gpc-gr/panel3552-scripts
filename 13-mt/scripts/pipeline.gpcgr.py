...
BWA_MEM    = '/u4/share1/home/gpc-gr/local/packages/bwa-0.7.12/bin/bwa'
SAMTOOLS   = '/u4/share1/home/gpc-gr/local/packages/samtools-1.6/samtools.sh'
JAVA       = '/usr/local/pkg/java/jdk1.8.0_144/bin/java -XX:+UseSerialGC'
GATK_JAR   = '/u4/share1/home/gpc-gr/local/packages/gatk-3.7-0-gcfedb67/GenomeAnalysisTK.jar'
...
def main_analysis(bam_file, sample_id, fasta_setting, output_dir, qsub_log_dir):
    ...
    vcf_file = '/'.join([output_dir, sample_id, 'vcf', sample_id + '.vcf'])
    ...
    vcf_file_with_shifted_mtDNA   = '/'.join([output_dir, sample_id, 'vcf', sample_id + '_shifted.vcf'])
    variant_call(bam_prefix_with_shifted_mtDNA + '.sorted.bam', reference, sample_id, vcf_file_with_shifted_mtDNA, qsub_log_dir)

    merged_vcf_file = '/'.join([output_dir, sample_id, 'vcf', sample_id + '_merged.vcf'])
...
def main(argv):
    ...
    sample_data_file = '/u4/share1/home/gpc-gr/panel-build37/work/11-single_sample/idtable-current.tsv'
    ...
    for i in xrange(len(data_item_list)):
        ...
        merged_vcf_file = 'debug_analysis/{tmmid}/vcf/{tmmid}_merged.vcf.gz'.format(tmmid=project_id)

        if (0 <= i < 4500) and (not os.path.exists(merged_vcf_file)):
